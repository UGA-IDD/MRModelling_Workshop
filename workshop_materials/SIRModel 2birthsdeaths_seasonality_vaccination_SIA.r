...
model <- function(brate=0.0001,drate=0.0001,gamma=0.1,mean_beta=.5, seasonality = 0.2,vaccov=0.8,vactime=500,vacdegree=1, SIAtime=seq(1000, tmax,by=3*365), SIAcov=.8,tmax=2000){
  # brate = birth rate
  # drate = death rate
  # gamma = recovery rate
  ...
  # SIAtime = a vector of times when SIAs occur; SIAs occur instantaneously in a day
  # SIAcov = coverage of the SIA; here assumed to cover all susceptibles, as this is not an age specific model
  ...
  for(t in (vactime+1):tmax){
    ...
    if(!is.na(match(t,SIAtime))){
    S[t+1]=S[t]-beta.t*S[t]*I[t]-drate*S[t]+brate*(S[t]+I[t]+R[t]+V[t])*(1-vaccov) - (S[t] * SIAcov)
    I[t+1]=I[t]+beta.t*S[t]*I[t]+beta.t*(1-vacdegree)*V[t]*I[t]-gamma*I[t]-drate*I[t]
    R[t+1]=R[t]+gamma*I[t]-drate*R[t]
    V[t+1]=V[t]+brate*(S[t]+I[t]+R[t]+V[t])*vaccov-beta.t*(1-vacdegree)*V[t]*I[t]-drate*V[t] + (S[t] * SIAcov)
    }
  }
...
#Example output
duration <- 100000
model.out <- model(tmax=duration, brate=0.001,drate=0.001, mean_beta=.5, seasonality=.5, vaccov=0.2, SIAtime=seq(1000,duration,by=5*365), SIAcov=.5) # run at baseline parameters
#plot.model(tmax=100)
...
# 1. change the SIA coverage to 0.8
# 2. change the SIA frequency to every 3 years
################################################################################
